<?php
	...
	//var_dump($settings);
	$col_desk = $settings[0]['col_desk_related_pages'] ?? '';
	$col_tab = $settings[0]['col_tab_related_pages'] ?? '';
	$col_mob = $settings[0]['col_mob_related_pages'] ?? '';
	$swiper = $settings[0]['truefalse_swiper_related_pages'] ?? '';
	$load_more = $settings[0]['truefalse_loadmore_related_pages'] ?? '';
	$read_more = $settings[0]['truefalse_learnmore_related_pages'] ?? '';
	$read_more_txt = $settings[0]['txt_learnmore_related_page'] ?? '';
	//var_dump($swiper);
?>
<!-- acf flexible - related pages -->
<div class="related-pages<?php if ($swiper == 'true'): ?> swiper-slider<?php endif; ?>">
	<div class="row related-pages-wrapper<?php if ($swiper == 'true'): ?> swiper-wrapper<?php endif; ?><?php if ($load_more == 'true'): ?> related__load-more--parent<?php endif; ?>">
		<?php
			foreach( $related_pages as $post ): //var_dump($post); 
				$post_id = $post->ID; 
